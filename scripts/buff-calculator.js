(function () {
  ...
  function scaleLevelValue(scale, buff) {
    ...
    if (source.type === "character") return Math.max(1, Number(buff.characterLevel || buff.level || buff.casterLevel || 1) || 1);
    if (source.type === "class") {
      const classLevels = buff.classLevels || {};
      return Math.max(1, Number(classLevels[source.className] || buff.classLevel || buff.casterLevel || 1) || 1);
    }
  ...
  function calculateStatsDetailed(activeBuffs, baseline) {
    const enrichedBuffs = (activeBuffs || []).map(buff => ({
      ...buff,
      characterLevel: buff.characterLevel || baseline.characterLevel || baseline.level,
